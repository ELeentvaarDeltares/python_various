...
# Remove counter
#new_elec = elec[:,1:] # It is not nessecary to do this because you do this again after fixing missing electrodes. 
...
# GUESS: load previous file or load synthetic data 
# out=np.load('out.npy')        
...
# DONTKNOWYET --- GUESS delete data points 
...
# Split the data into different lines based on data locations in abmn, this is abmn split in 4 data lines
# TODO: WHY IS NONE IF THIS USED LATER????
test1 = np.c_[abmn[d1,:]]
test2 = np.c_[abmn[d2,:]]
test3 = np.c_[abmn[d3,:]]
test4 = np.c_[abmn[d4,:]]

# Find mean of all data parts
...
# Fill created array with only coordinates
test1c[:,:4] = test1[:,:4]
test2c[:,:4] = test2[:,:4]
test3c[:,:4] = test3[:,:4]
test4c[:,:4] = test4[:,:4]
